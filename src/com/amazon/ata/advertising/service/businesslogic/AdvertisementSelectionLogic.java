...
public class AdvertisementSelectionLogic {
    ...
    public GeneratedAdvertisement selectAdvertisement(String customerId, String marketplaceId) {
        ...
        if (StringUtils.isEmpty(marketplaceId)) {
            ...
            Map<Double, AdvertisementContent> advertisementContentMap = new TreeMap<>(Comparator.reverseOrder());

            contentDao.get(marketplaceId)
                    .forEach(adContent -> (
                            targetingGroupDao.get(adContent.getContentId())
                                    .stream()
                                    .filter(targetingGroup -> targetingEvaluator
                                            .evaluate(targetingGroup).isTrue()))
                                    .forEach(x -> advertisementContentMap.put(x.getClickThroughRate(), adContent)));

            generatedAdvertisement = advertisementContentMap.values()
                    .stream()
                    .findFirst()
